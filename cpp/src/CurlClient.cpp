...
namespace CurlService
{
    ...
    CurlClient::CurlClient(EventBase* evb,
        ...
        evb_(evb), httpMethod_(httpMethod), url_(url), inputFilename_(inputFilename), h2c_(h2c),
        httpMajor_(httpMajor), httpMinor_(httpMinor)
    {
        if (proxy != nullptr)
        {
    ...
    {
        LOG_IF(INFO, loggingEnabled_) << fmt::format("Sending request for {}", url_.getUrl());
        txn_ = txn;
        setupHeaders();
        ...
        {
            inputFile_ = std::make_unique<ifstream>(inputFilename_, ios::in | ios::binary);
            sendBodyFromFile();
        }
        else
        {
    ...
    void CurlClient::printMessageImpl(proxygen::HTTPMessage* msg, const std::string& tag)
    {
        if (!loggingEnabled_)
        {
            return;
    ...
    void CurlClient::onPushedTransaction(proxygen::HTTPTransaction* pushedTxn) noexcept
    {
        //
    ...
    void CurlClient::CurlPushHandler::setTransaction(proxygen::HTTPTransaction* txn) noexcept
    {
    ...
    void CurlClient::CurlPushHandler::onBody(std::unique_ptr<folly::IOBuf> chain) noexcept
    {
    ...
    void CurlClient::CurlPushHandler::onError(const proxygen::HTTPException& error) noexcept
    {
