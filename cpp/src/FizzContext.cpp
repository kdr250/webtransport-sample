...
namespace quic::samples
{
    FizzServerContextPtr createFizzServerContext(const MyHQServerParams& params)
    {
        ...
        auto cert2 =
            fizz::openssl::CertUtils::makeSelfCert(kPrime256v1CertData, kPrime256v1KeyData);
    ...
    FizzClientContextPtr createFizzClientContext(const MyHQBaseParams& params, bool earlyData)
    {
        ...
        ctx->setDefaultShares({fizz::NamedGroup::x25519, fizz::NamedGroup::secp256r1});
        ctx->setSendEarlyData(earlyData);
        auto mgr = std::make_shared<fizz::CertDecompressionManager>();
        mgr->setDecompressors({std::make_shared<fizz::ZstdCertificateDecompressor>(),
